...
type
  ...
  { TFreePropeller_Task3 }

  TFreePropeller_Task3 = class(TForm)
...
var
  FreePropeller_Task3: TFreePropeller_Task3;
...
begin
  Edit14.Value := val;
  if (Edit14.Value = 0) or (Edit14.Value > 1) then
    Edit14.Value := 0.98;
...
begin
  Edit15.Value := val;
  if (Edit15.Value = 0) or (Edit15.Value > 1) then
    Edit15.Value := 0.98;
...
begin
  ...
  ResultsMemo2.Lines.Add(rs_Propeller_Task2_Note);
  ResultsMemo2.Lines.Add('');
  ResultsMemo2.Lines.Add('Copyright (c) 2007, Timoshenko V.F.');
  ResultsMemo2.Visible := True;

  if (Dat2 <= 0) or (Dat3 <= 0) or (Dat4 <= 0) or (Dat5 <= 0) or (Dat13 <= 0) or
    (Dat14 <= 0) or (Dat15 <= 0) or (Dat7 > 17) or (Dat6 <= 0) or (Dat7 <= 0) or
  ...
  begin
    ...
    FileToFind := FileSearchUTF8('TMP3.tsk', GetCurrentDir);
...
{$I+}
  ...
  begin
    ...
    begin
      for I := 1 to 5 do
      begin
        if res[i, j + ji] >= 10000 then
          ss[i] := FloatToStrF(res[i, j + ji], ffFixed, 6, 0) + '  | ';
        if (res[i, j + ji] >= 1000) and (res[i, j + ji] < 10000) then
          ss[i] := FloatToStrF(res[i, j + ji], ffFixed, 6, 0) + '.  | ';
        if (res[i, j + ji] >= 100) and (res[i, j + ji] < 1000) then
          ss[i] := FloatToStrF(res[i, j + ji], ffFixed, 6, 1) + '  | ';
        if (res[i, j + ji] >= 10) and (res[i, j + ji] < 100) then
          ss[i] := FloatToStrF(res[i, j + ji], ffFixed, 6, 2) + '  | ';
        if res[i, j + ji] < 10 then
          ss[i] := FloatToStrF(res[i, j + ji], ffFixed, 6, 3) + '  | ';
      end;
  ...
  Km := 1;
  Kt := 1;
  if Res[1, 33] / Res[1, 32] > 2 then
    Kt := 2;
  if Res[1, 33] / Res[1, 32] > 5 then
    Kt := 5;
  if Res[1, 33] / Res[1, 32] > 10 then
    Kt := 10;
  Kk := 381;
  if Res[5, 33] < 5 then
  begin
  ...
  if Kt > 1 then
    Chart.LeftAxis.Title.Caption := Userstring(Kk) + '*' + FloatToStrF(kt, ffFixed, 5, 0)
  else
    Chart.LeftAxis.Title.Caption := Userstring(Kk);
...
//     ResultsMemo.Lines.Add(FloatToStrF(Ys,ffFixed,6,3));
  ...
  begin
    ...
    begin
      ...
      nn := res[1, 5 + ji];
      if nn >= 10000 then
        ss[j] := FloatToStrF(nn, ffFixed, 6, 0) + '  | ';
      if (nn >= 1000) and (nn < 10000) then
        ss[j] := FloatToStrF(nn, ffFixed, 6, 0) + '.  | ';
      if (nn >= 100) and (nn < 1000) then
        ss[j] := FloatToStrF(nn, ffFixed, 6, 1) + '  | ';
      if (nn >= 10) and (nn < 100) then
        ss[j] := FloatToStrF(nn, ffFixed, 6, 2) + '  | ';
      if nn < 10 then
...

{
    ...
    if yy[1] > Res[1, 11] * 0.7 then
      Series_11.AddXY(yy[1], -xx[1], '', clRed);
    if yy[2] > Res[1, 11] * 0.7 then
      Series_11.AddXY(yy[2], -xx[2], '', clRed);
    Series_11.AddXY(yy[3], -xx[3], '', clRed);
    if yy[4] < Res[3, 26] then
      Series_11.AddXY(yy[4], -xx[4], '', clRed)
    else
      Series_11.AddXY(Res[3, 26], -Res[3, 28] * Km, '', clRed);
...
{$I+}
  ...
  if yy[1] > Res[1, 11] * 0.7 then
    Series_12.AddXY(yy[1], tt[1], '', clRed);
  if yy[2] > Res[1, 11] * 0.7 then
    Series_12.AddXY(yy[2], tt[2], '', clRed);
  Series_12.AddXY(yy[3], tt[3], '', clRed);
  if yy[4] < Res[3, 26] then
    Series_12.AddXY(yy[4], tt[4], '', clRed)
  else
    Series_12.AddXY(Res[3, 26], Res[3, 27] * Km * Kt, '', clRed);
