...
const
  Matrix1: array[0..13, 0..9] of
    extended = ((-6.735654, 38.368310, -0.008193, 0.055234, -1.997242, -38.860810,
    0.956591, -0.002171, 0.272895, -0.017516),
    (-0.382870, 38.172900,
    0.007243, 0.026644, -5.295332, -39.550320, 1.219563, 0.000052, 0.824568, -0.047842),
    (-1.503526, 24.408030,
    0.012200, 0.067221, -2.448582, -31.913700, 2.216098, 0.000074, 0.244345, -0.015887),
    (11.292180, -14.519470,
    0.047182, 0.085176, -2.673016, -11.418190, 5.654065, 0.007021, -0.094934, 0.006325),
    (22.178670, -49.167840,
    0.085998, 0.150725, -2.878684, 7.167049, 8.600272, 0.012981, -0.327085, 0.018271),
    (25.908670, -74.756680,
    0.153521, 0.188568, -0.889467, 24.121370, 10.485160, 0.025348, -0.854940, 0.048449),
    (40.975590, -114.285500,
    0.207226, 0.250827, -3.072662, 53.015700, 13.021770, 0.035943, -0.715457, 0.039874),
    (45.837590, -184.764600,
    0.357031, 0.338343, 3.871658, 132.256800, 10.860540, 0.066809, -1.719215, 0.095977),
    (89.203820, -393.0127,
    0.617466, 0.460472, 11.54327, 331.1197, 8.598136, 0.104073, -2.815203, 0.155960),
    (
    212.678800, -801.790800, 1.087307, 0.538938, 10.802730, 667.644500, 12.398150,
    0.166473, -3.026131, 0.165055),
    (
    336.235400, -1085.134000, 1.644191, 0.532702, -1.224173, 831.144500, 26.183210,
    0.238795, -2.450470, 0.139154),
    (
    566.547600, -1609.632000, 2.016090, 0.265722, -29.244120, 1154.091, 51.46575,
    0.288046, -0.178354, 0.018446),
    (
    743.410700, -1708.263000, 2.435809, 0.013553, -81.161890, 937.401400,
    115.600600, 0.365071, 1.838967, -0.062023),
    (1200.62, -2751.715,
    3.208577, 0.254920, -132.0424, 1489.269, 196.3406, 0.528225, 1.379102, 0.013577));
  Matrix2: array[0..11, 0..5] of
    extended = ((180.100400, -31.502570, -7.451141, 2.195042, 2.689623, 0.006480),
    (243.999400, -44.525510,
    -11.154560, 2.179046, 3.857403, 0.009676),
    (282.987300, -51.519530,
    -12.973100, 2.274505, 4.343662, 0.011066),
    (313.410900, -56.582570,
    -14.419780, 2.326117, 4.690432, 0.012147),
    (337.003800, -59.190290,
    -16.069750, 2.419156, 4.766793, 0.014147),
    (356.457200, -62.853950,
    -16.851120, 2.437056, 5.078768, 0.014980),
    (324.735700, -51.312520,
    -15.345950, 2.334146, 3.855368, 0.013695),
    (301.126800, -39.796310,
    -15.022990, 2.059657, 2.545676, 0.013588),
    (292.057100, -31.853030,
    -15.585480, 1.847926, 1.569917, 0.014014),
    (284.464100, -25.145580,
    -16.154230, 1.703981, 0.817921, 0.014575),
    (256.636700, -19.319220,
    -13.084500, 2.152824, 0.348305, 0.011343),
    (304.180300, -30.115120,
    -15.854290, 2.863173, 1.524379, 0.014031));
  Matrix3: array[0..13, 0..9] of
    extended = ((-6.735654, 38.368310, -0.008193, 0.055234, -1.997242, -38.860810,
    0.956591, -0.002171, 0.272895, -0.017516),
    (-0.382870, 38.172900,
    0.007243, 0.026644, -5.295332, -39.550320, 1.219563, 0.000052, 0.824568, -0.047842),
...
type
  ...
  TFreeResistance_Delft = class(TForm)
    ...
    ChartAxisTransformations1:
    TChartAxisTransformations;

    ChartAxisTransformations1AutoScaleAxisTransform1:
    TAutoScaleAxisTransform;

    ChartAxisTransformations2:
    TChartAxisTransformations;

    ChartAxisTransformations2AutoScaleAxisTransform1:
    TAutoScaleAxisTransform;
    CheckBox2: TCheckBox;
  ...
  private
    ...
    procedure CalculateResistance(
      ConvertedSpeed, LCB, Cp: single; var Rf, Rr, Rt: single);
...
var
  FreeResistance_Delft: TFreeResistance_Delft;
...
begin
  ...
  if (Draft <= 0.02) or (Lwl <= 0) or (Bwl <= 0) or (DraftTotal < Draft) or
    ...
    (Cp <= 0) or (Viscosity <= 0) or (EndSpeed <= StartSpeed) or (StepSpeed <= 0) then
    exit;
...
begin
  ...
  begin
    ...
    ResultsMemo.Lines.Add(Space(10) + 'Bwl/Tc     = ' + FloatToStrF(
      Bwl / Draft, ffFixed, 6, 3));
    ResultsMemo.Lines.Add(Space(10) + 'Lwl/V^3    = ' + FloatToStrF(
      Lwl / Power(Displacement, 1 / 3), ffFixed, 6, 3));
    ResultsMemo.Lines.Add(Space(10) + 'Tc/T       = ' + FloatToStrF(
      Draft / DraftTotal, ffFixed, 6, 3));
    ResultsMemo.Lines.Add(Space(10) + 'Aw/V^0.667 = ' + FloatToStrF(
      WlArea / Power(Displacement, 2 / 3), ffFixed, 7, 3));
    ResultsMemo.Lines.Add('');
    for ii := 1 to 12 do
      ResultsMemo.Lines.Add('');
    if StepSpeed = 0.0 then
      StepSpeed := 0.1;
...
procedure TFreeResistance_Delft.CalculateResistance(ConvertedSpeed, LCB, Cp: single;
  var Rf, Rr, Rt: single);
var
  Rn_hull, Cf_hull, Rf_hull: single;
...
{      ResultsMemo.Lines.Add('Re/10^8='+FloatToStrF(Rn_hull/100000000,ffFixed,6,4));
  ...
  begin
    ...
    begin
      ...
      for a := 0 to 9 do
        factors[a] := fraction * (Matrix1[0, a]);
    ...
    begin
      ...
      for a := 0 to 9 do
        factors[a] := Matrix1[Lower, a] + fraction * (Matrix1[Lower + 1, a] - Matrix1[Lower, a]);
    end;
    Rr := 9.81 * Displ * WaterDensity * (factors[0] + factors[1] * Cp +
      factors[2] * LCB + factors[3] * B_T + factors[4] * L_D +
      factors[5] * Cp * Cp + factors[6] * Cp * L_D +
      factors[7] * LCB * LCB + factors[8] * L_D * L_D +
      factors[9] * L_D * L_D * L_D);
  end
  else if (FroudeNumber > 0.475) and (FroudeNumber <= 0.75) then
  begin
    Lower := trunc((FroudeNumber - 0.475) / 0.025);
    Fraction := (FroudeNumber - (0.475 + Lower * 0.025)) / 0.025;
    for a := 0 to 5 do
      factors[a] := Matrix2[Lower, a] + fraction * (Matrix2[Lower + 1, a] - Matrix2[Lower, a]);
    Rr := Displ * WaterDensity * (factors[0] + factors[1] * L_B +
      factors[2] * A_D + factors[3] * LCB + factors[4] * L_B * L_B +
...
procedure TFreeResistance_Delft.CalculateResistanceHoltr(ConvertedSpeed, LCB, Cp: single;
...
begin
  ...
  begin
    ...
    if DraftTotal - Draft = 0 then
      DraftTot := DraftTotal * Foot
    else
      DraftTot := (DraftTotal - Draft) * Foot;
    WaterDensity := Density / WeightConversionFactor;
    Keel_chord := KeelChordLength * Foot;
    Keel_Area := KeelArea * Foot * Foot;
    Rudder_chord := RudderChordLength * Foot;
...
procedure TFreeResistance_Delft.resist(vms: single; dat: array of single;
  var Rf, Rr, w, t0, nr: single);
const
  appc: array[1..11] of extended = (1.75, 1.4, 2.8, 3.0, 1.75, 3.0, 2.0, 3.0, 2.8, 2.7, 1.4);
...
begin
  ...
  begin
    Ae0 := 0.55;
    t0 := 0.25014 * power(B_L, 0.28956) * power(
      (sqrt(B * T) / D), 0.2624) / power((1 - Cp + 0.0225 * lcb), 0.01762) + 0.0015 * Cstrn;
  ...
  begin
    if Np = 1 then
      w := C[9] * C[20] * Cv * L / Ta * (0.050776 + 0.93405 * C[11] * Cv / (1 - Cp1)) + 0.27915 *
        C[20] * sqrt(B / (L * (1 - Cp1))) + C[19] * C[20]
    else
