...
type
  ...
  TFreeResistance_Hollen = class(TForm)
...
var
  FreeResistance_Hollen: TFreeResistance_Hollen;
...
begin
  ...
  {for i := 711 to 724 do
    ResultsMemo2.Lines.Add(Userstring(i));}
  ResultsMemo2.Lines.Add(rs_Resistance_Hollen_Note_p5);
...
*)
    ...
    ResultsMemo.Lines.Add(Space(14) + 'L/B           = ' + FloatToStrF(
      Lpp / Bwl, ffFixed, 6, 3));
    ResultsMemo.Lines.Add(Space(14) + 'Los/Lwl       = ' + FloatToStrF(
      Los / Lwl, ffFixed, 6, 3));
    ResultsMemo.Lines.Add(Space(14) + 'Lwl/L         = ' + FloatToStrF(
      Lwl / Lpp, ffFixed, 6, 3));
      ...
      begin
        ...
        begin
          ...
          MessageDlg(rs_Do_NOT_calculate_this_task__because_ {UserString[1138]}
           + #13#10#13#10 + rs_1__Do_not_found_into__Exec_or_was_damaged_file {UserString[1139]} + ' Hollenbh.EXE '
...
//{$I+}
      ...
      begin
        ...
        begin
          ...
          // Мощность
          Series5.AddXY(res[1, II], (Rr_min[I] + res[6, II]), '', clTeeColor);
...
procedure TFreeResistance_Hollen.FSetA11(val: integer);
begin
  A11box.ItemIndex := val - 1;
end;{TFreeResistance_Hollen.FSetA11}

function TFreeResistance_Hollen.FGetA11: integer;
begin
...
begin
  ...
  if (Dpbox.Value = 0) and (Np = 2) then
    Dpbox.Value := (DraftTotal - Draft / 2) * 0.7;
...
begin
  ...
  if ZBulb > 0 then
    Nballast := 1;
  if (RudderChordLength > 0) and (RudderArea = 0) then
    Nrud := 1;
  if (RudderArea > 0) and (RudderChordLength = 0) then
    Nrud := 2;
  if (RudderArea > 0) and (RudderChordLength > 0) then
    Nrud := 3;
  if KBulb > 0 then
  ...
  if Np <= 1 then
  begin
    ResultsMemo.Lines.Add(Space(14) + 'Nballast      = ' +
      FloatToStrF(Nballast, ffFixed, 6, 0));
  ...
  begin
    ...
    ResultsMemo.Lines.Add(Space(14) + 'Nthruster     = ' +
      FloatToStrF(Nthrus, ffFixed, 6, 0));
    ResultsMemo.Lines.Add(Space(14) + 'Nbulb         = ' +
      FloatToStrF(Nbulb, ffFixed, 6, 0));
    ResultsMemo.Lines.Add(Space(14) + 'Nbrackets     = ' +
      FloatToStrF(Nbrac, ffFixed, 6, 0));
    ResultsMemo.Lines.Add(Space(14) + 'Nbossings     = ' +
      FloatToStrF(Nboss, ffFixed, 6, 0));
    ResultsMemo.Lines.Add(Space(14) + 'Nrudders      = ' +
      FloatToStrF(Nrud, ffFixed, 6, 0));
  end;
