...
{ TFreeResistance_MH }
 ...
 TFreeResistance_MH   = class(TForm)
				    ...
				    Estimate2Box: TCheckBox;
                                    ...
                                    CpBox: TFreeNumInput;
                                    KeBox: TFreeNumInput;
                                    NserBox: TFreeNumInput;
                                    NfBox: TFreeNumInput;
                                    NaBox: TFreeNumInput;
                                    NprotBox: TFreeNumInput;
                                    NpBox: TFreeNumInput;
                                    DpBox: TFreeNumInput;
                                    KsBox: TFreeNumInput;
                                    K1Box: TFreeNumInput;
...
var FreeResistance_MH: TFreeResistance_MH;
...
begin
   ...
   if (DraftTotal<=0.02)  or  (Lwl<=0) or (Bwl<=0)  or (Displacement<=0) or (Cp<=0) or
      (Viscosity<=0) Or (EndSpeed<=StartSpeed) or (StepSpeed<=EndSpeed) then exit;
...
begin
   Nser := self.Nser;
       ...
       if Combobox.ItemIndex=1 then Nser:=14;
       if Combobox.ItemIndex=2 then Nser:=15;
       if Combobox.ItemIndex=3 then Nser:=19;
       if Combobox.ItemIndex=4 then Nser:=20;
       if Combobox.ItemIndex=5 then Nser:=21;
       if Combobox.ItemIndex=6 then Nser:=22;
       if Combobox.ItemIndex=7 then Nser:=23;
       if Combobox.ItemIndex=8 then Nser:=24;

   Vmin:=0.1*sqrt(9.81*Lwl)/0.51444;
   if Nser=14 then Vmin:=0.8*sqrt(9.81*power(Displacement,0.3333))/0.51444;
   if Nser=15 then Vmin:=Vmin*2.5;
   if Nser=19 then Vmin:=Vmin*4;
...
//   Расчет на макс скорости для определения масштабного коэффициента flag
    ...
    FileName:='ostres.tmp';
     ...
     if FileExistsUTF8(PathFileOld+'ostres.tmp') { *Converted from FileExists* } then  begin
      ...
      {$I-}Reset(FFile);{$I+}
         ...
         // Удаление временных файлов
         	  if FileExistsUTF8(PathFileOld+'ostres.tmp') { *Converted from FileExists* } then  begin
         	          DeleteFileUTF8(PChar(PathFile+'/Resist.dat')); { *Converted from DeleteFile* }
          		  DeleteFileUTF8(PChar(PathFile+'/Resistp.dat')); { *Converted from DeleteFile* }
         		  ...
         		  L:=RenameFileUTF8(PChar(PathFileOld+'Resistp.dat'),PChar(PathFile+'/Resistp.dat'));
         	          DeleteFileUTF8(PChar(PathFileOld+'ostres.tmp')); { *Converted from DeleteFile* }
          		  DeleteFileUTF8(PChar(PathFileOld+'Resist.dat')); { *Converted from DeleteFile* }
         	  end;
       ...
       Index:=0;
        ...
        For ispeed:=1 to 10 do
         begin
            ...
            if ((FroudeNumber>=0) and (FroudeNumber<=0.6) or (Nser=14) or (Nser=15) or (Nser=19)) then
            begin
           ...
           if Nser=19 then Chart.Title.Text.Text:=Userstring(265)+' '+Userstring(502);		   
            ...
            if ispeed=8 then begin
               ...
               If (Tb<=0) then begin
				 ...
				 if (Nser>0) and (Nser<20) then MessageDlg(Userstring(487),mtError,[mbOk],0);
                 ResultsMemo.Lines.Add('');
               end; 
               Vms:=ConvertedSpeed;
            end;
...
//   ViscosityBox.Enabled:=False;
   ...
   Checkbox2.Checked:=AutoExtract;
   	...
   	if Nser=0 then Combobox.ItemIndex:=0;   
        if Nser=14 then Combobox.ItemIndex:=1;
	if Nser=15 then Combobox.ItemIndex:=2;
	if Nser=19 then Combobox.ItemIndex:=3;   
	if Nser=20 then Combobox.ItemIndex:=4;   
	if Nser=21 then Combobox.ItemIndex:=5;   
	if Nser=22 then Combobox.ItemIndex:=6;   
	if Nser=23 then Combobox.ItemIndex:=7;   
	if Nser=24 then Combobox.ItemIndex:=8;   
...
procedure TFreeResistance_MH.CalculateResistanceOST(Vs: array of single; LCB,Cp:single;var Rf,Rr,w,t0,nr: single);
var
...
//	  dat[28]:=Np;        
	...
	FrVmax:=FrVmin*Vs[9]/Vs[0];
      ...
      if Nser=15 then begin 
             ResultsMemo.Lines.Add(Space(10)+'Nh            = '+FloatToStrF(Np,ffFixed,6,0));
			 if (Dp<Bwl+1) and (Np>1) then Dp:=Bwl+1;
             c_L:=Dp/Lwl; 			 
             ...
             if (c_L>0.45) and (Np>1) then begin
   ...
   if (FrVmin<0.5) and (Nser=14) then begin
       exit;
   end;
...
var I          : integer;
    ffile      : textfile;
begin
     if FileExistsUTF8(PathFileOld+'ostdat.tmp') { *Converted from FileExists* } then  DeleteFileUTF8(PChar(PathFile+'\ostdat.tmp')); { *Converted from DeleteFile* }
...
procedure TFreeResistance_MH.FSetA11(val:single);
begin
   A11box.Value:=val;
end;{TFreeResistance_MH.FSetA11}

function TFreeResistance_MH.FGetA11:single;
begin
   Result:=A11box.Value;
