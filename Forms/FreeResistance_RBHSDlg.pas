...
var
  FreeResistance_RBHS: TFreeResistance_RBHS;

implementation

uses
  FreeStringsUnit, FreeLogger, FreeProcess;
...
begin
  ...
  if (Nser = 5) or (Nser = 10) or (Nser = 13) or (Nser = 14) then
    Vmin := Vmin / 0.5 * 0.8;
  if (Nser = 8) or (Nser = 9) or (Nser = 11) then
    Vmin := Vmin / 0.5;
...
procedure TFreeResistance_RBHS.CalculateResistanceOST(Vs: array of single; LCB, Cp: single;
...
begin
  ...
  if (Nser = 7) or (Nser = 8) or (Nser = 9) or (Nser = 10) or (Nser = 11) or
    (Nser = 12) or (Nser = 13) or (Nser = 14) then
  begin
    NfBox.Enabled := False;
    NaBox.Enabled := False;
  end;
  ...
  begin
    ...
    ResultsMemo.Lines.Add(
      '(FrVmin<0.5) or (FrVmax>2.8)) and ((Nser=5) or ((Nser>=8) and (Nser<=14)): Exit');
    exit;
  end;
    ...
    begin
      ...
      begin
        ResultsMemo.Lines.Add('Error: Fr[' + IntToStr(i) +
          ']<=0) or Cr[' + IntToStr(i) + ']<=0) : exit');
        exit;
      end;
      if (dan[15 + i] >= dan[16 + i]) and (i < 5) then
      begin
        ResultsMemo.Lines.Add('Error: Fr[' + IntToStr(i) + '] >= Fr[' +
          IntToStr(i + 1) + '] : exit');
        exit;
      end;
    end;
...
procedure TFreeResistance_RBHS.FSetA11(val: integer);
begin
  A11box.ItemIndex := val - 1;
end;{TFreeResistance_RBHS.FSetA11}

function TFreeResistance_RBHS.FGetA11: integer;
begin
