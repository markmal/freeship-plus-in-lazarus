...
type TFreeHydrostaticsForm  = class(TForm)
                                    Panel2: TPanel;
...
begin
   Edit1.Text:=FloatToStrF(Val,ffFixed,7,3);
...
begin
   Edit2.Text:=FloatToStrF(Val,ffFixed,7,3);
...
begin
   ...
   begin
      ...
      try
         ...
         for I:=0 to 16 do if I in [0,1,2,3,6,7,8,10,12,14] then ResultsDlg.Grid.ColWidths[I]:=47 else
                           if I in [4,5,15] then ResultsDlg.Grid.ColWidths[I]:=55;
         Zmin:=0;
         I:=2;
...
// Пересчет коэфФициентов полноты, если базовая линия проходит через линию киля
         If abs(HydObject.Data.ModelMin.Z)>0.001 then begin
          ...
          Cm:=HydObject.Data.MainframeCoeff*HydObject.Draft/(HydObject.Draft+HydObject.Data.ModelMin.Z);
         end else begin
          ...
          if Cm>0 then begin 
            ...
            ResultsDlg.Grid.Cells[ 9,I]:=FloatToStrF(HydObject.Data.Mainframearea,ffFixed,7,3);
...
procedure TFreeHydrostaticsForm.FormShow(Sender: TObject);
begin
   Edit1.SetFocus;
end;
